...
fn compute_query_cost(query: &String, word_bank: &Vec<String>) -> (usize, HashMap<Filter, Vec<String>>) {
    ...
    // let mut max_count: usize = 0;
    // let mut max_count_secret_candidates: Vec<String> = Vec::new();
    // for (filter, secret_candidates_for_filter) in filters_to_secret_candidates_for_query.iter() {
    //     // cost is worst-case performance
    //     if secret_candidates_for_filter.len() > max_count {
    //         max_count = secret_candidates_for_filter.len();
    //         max_count_secret_candidates = secret_candidates_for_filter.clone();
    //     }
    // }
    // (max_count, max_count_secret_candidates, filters_to_secret_candidates_for_query)
...
fn main() {
    ...
    // println!("{}", compute_filter(&String::from("silly"), &String::from("skill")));
    // return;
