...
fn main() {
    ...
    // println!("final filter: {}", compute_filter(&Word::new("oooll"), &Word::new("llool")));
    // return;

    // Main application
    // TODO: Check that word bank exists by returning a Result from get_word_bank
    let word_bank = wordle::get_word_bank(&arg_path_queries);
    let mut secret_candidates = wordle::get_word_bank(&arg_path_solutions);
